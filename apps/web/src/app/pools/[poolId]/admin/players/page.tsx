...
export default function AdminPlayersPage() {
  ...
  const selectedGroupMatch = Object.values(matchesByGroup)
    .flat()
    .find(
      (match) => selectedMatch?.matchType === 'group' && match.matchId === selectedMatch.matchId
    );
  const selectedFinalMatch = Object.values(bracket)
    .flat()
    .find(
      (match) =>
        selectedMatch?.matchType === 'final' && match.bracketMatchId === selectedMatch.matchId
    );
  ...
  const selectedCountryOption =
    countryOptions.find((option) => option.value === playerCountryFilter) ?? null;
  ...
  const selectedPositionOption =
    positionOptions.find((option) => option.value === playerPositionFilter) ?? null;
  ...
  const selectedMatchOption =
    matchOptions.find(
      (option) =>
        option.matchId === selectedMatch?.matchId && option.matchType === selectedMatch.matchType
    ) ?? null;
  ...
  const parseNonPositive = (value: string) =>
    parseConfigNumberInput(value, { allowNegative: true, max: 0 });
        ...
        <Section
          title={
            <span className="admin-section-title">
              ...
              {t('adminResults.players.selectionLimits.title')}
            </span>
          }
          collapsible
          defaultExpanded
      ...
      {systemMode ? null : (
        ...
        >
          ...
          >
            ...
            {[
              {
                labelKey: 'adminResults.config.players.subgroups.goalsByPosition',
                icon: <FaFutbol style={{ color: 'rgb(var(--fg))' }} />,
                fields: [
                  {
                    label: t('adminResults.players.positions.goalkeeper'),
                    value: playerScoringConfig.goal.goalkeeper,
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({ ...p, goal: { ...p.goal, goalkeeper: v } })),
                  },
                  {
                    label: t('adminResults.players.positions.defender'),
                    value: playerScoringConfig.goal.defender,
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({ ...p, goal: { ...p.goal, defender: v } })),
                  },
                  {
                    label: t('adminResults.players.positions.midfielder'),
                    value: playerScoringConfig.goal.midfielder,
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({ ...p, goal: { ...p.goal, midfielder: v } })),
                  },
                  {
                    label: t('adminResults.players.positions.forward'),
                    value: playerScoringConfig.goal.forward,
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({ ...p, goal: { ...p.goal, forward: v } })),
                  },
                ],
              },
              {
                labelKey: 'adminResults.config.players.subgroups.assistsByPosition',
                ...
                fields: [
                  {
                    ...
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        assist: { ...p.assist, goalkeeper: v },
                      })),
                  },
                  {
                    label: t('adminResults.players.positions.defender'),
                  ...
                  },
                  {
                    ...
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        assist: { ...p.assist, forward: v },
                      })),
                  },
                ],
              },
              {
                labelKey: 'adminResults.config.players.subgroups.cleanSheetsByPosition',
                ...
                fields: [
                  {
                    ...
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        cleanSheet: { ...p.cleanSheet, goalkeeper: v },
                      })),
                  },
                  {
                    label: t('adminResults.players.positions.defender'),
                    ...
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        cleanSheet: { ...p.cleanSheet, defender: v },
                      })),
                  },
                  {
                    label: t('adminResults.players.positions.midfielder'),
                    ...
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        cleanSheet: { ...p.cleanSheet, midfielder: v },
                      })),
                  },
                  {
                    label: t('adminResults.players.positions.forward'),
                    ...
                    onChange: (v: ConfigNumber) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        cleanSheet: { ...p.cleanSheet, forward: v },
                      })),
                  },
                ],
              },
                  ...
                  {fields.map((field) => (
                    <Field key={field.label} label={field.label}>
                      <Input
                        ...
                        onChange={(e) => field.onChange(parsePositive(e.target.value))}
                      />
                    </Field>
                  ))}
                </div>
              ...
              <Field
                label={
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    {t('adminResults.players.scoring.mvp')}
                  </span>
                }
              >
                <Input
                  ...
                  attention={playerScoringConfig.mvp === ''}
                  onChange={(e) =>
                    setPlayerScoringConfig((p) => ({ ...p, mvp: parsePositive(e.target.value) }))
                  }
                />
              </Field>
              <Field
                label={
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    {t('adminResults.players.scoring.yellowCard')}
                  </span>
                }
              >
                <Input
                  ...
                  attention={playerScoringConfig.yellowCard === ''}
                  onChange={(e) =>
                    setPlayerScoringConfig((p) => ({
                      ...p,
                      yellowCard: parseNonPositive(e.target.value),
                    }))
                  }
                />
              </Field>
              <Field
                label={
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    {t('adminResults.players.scoring.doubleYellowCard')}
                  </span>
                }
              >
                <Input
                  ...
                  inputMode="numeric"
                  max="0"
                  value={playerScoringConfig.doubleYellowCard}
                  attention={playerScoringConfig.doubleYellowCard === ''}
                  onChange={(e) =>
                    setPlayerScoringConfig((p) => ({
                      ...p,
                      doubleYellowCard: parseNonPositive(e.target.value),
                    }))
                  }
                />
              </Field>
              <Field
                label={
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    />
                    {t('adminResults.players.scoring.redCard')}
                  </span>
                }
              >
                <Input
                  ...
                  attention={playerScoringConfig.redCard === ''}
                  onChange={(e) =>
                    setPlayerScoringConfig((p) => ({
                      ...p,
                      redCard: parseNonPositive(e.target.value),
                    }))
                  }
                />
              </Field>
            ...
            <div style={actionGroupStyle}>
                  ...
                  {[
                    ...
                    <Field key={position} label={label}>
                      <Input
                        ...
                        onChange={(event) =>
                          setPlayerScoringConfig((previous) => ({
                            ...previous,
                            penaltyGoal: {
              ...
              >
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <GiGoalKeeper style={{ color: 'rgb(var(--fg))' }} />
                      {t('adminResults.players.scoring.penaltySaved')}
                    </span>
                  }
                >
                  <Input
                    ...
                    attention={playerScoringConfig.penaltySaved === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        penaltySaved: parsePositive(e.target.value),
                      }))
                    }
                  />
                </Field>
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <IoMdCloseCircle style={{ color: 'rgb(var(--live))' }} />
                      {t('adminResults.players.scoring.missedPenalty')}
                    </span>
                  }
                >
                  <Input
                    ...
                    attention={playerScoringConfig.missedPenalty === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        missedPenalty: parseSigned(e.target.value),
                      }))
                    }
                  />
                </Field>
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <IoMdCloseCircle style={{ color: 'rgb(var(--fg))' }} />
                      {t('adminResults.players.scoring.forcedPenaltyMiss')}
                    </span>
                  }
                >
                  <Input
                    ...
                    inputMode="numeric"
                    min="0"
                    value={playerScoringConfig.forcedPenaltyMiss}
                    attention={playerScoringConfig.forcedPenaltyMiss === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        forcedPenaltyMiss: parsePositive(e.target.value),
                      }))
                    }
                  />
                </Field>
              ...
              >
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <FaFutbol style={{ color: 'rgb(var(--fg))' }} />
                      {t('adminResults.players.scoring.shootoutGoal')}
                    </span>
                  }
                >
                  <Input
                    ...
                    inputMode="numeric"
                    min="0"
                    value={playerScoringConfig.shootoutGoal}
                    attention={playerScoringConfig.shootoutGoal === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        shootoutGoal: parsePositive(e.target.value),
                      }))
                    }
                  />
                </Field>
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <GiGoalKeeper style={{ color: 'rgb(var(--fg))' }} />
                      {t('adminResults.players.scoring.shootoutPenaltySaved')}
                    </span>
                  }
                >
                  <Input
                    ...
                    inputMode="numeric"
                    min="0"
                    value={playerScoringConfig.shootoutPenaltySaved}
                    attention={playerScoringConfig.shootoutPenaltySaved === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        shootoutPenaltySaved: parsePositive(e.target.value),
                      }))
                    }
                  />
                </Field>
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <IoMdCloseCircle style={{ color: 'rgb(var(--live))' }} />
                      {t('adminResults.players.scoring.shootoutMissedPenalty')}
                    </span>
                  }
                >
                  <Input
                    type="number"
                    inputMode="numeric"
                    value={playerScoringConfig.shootoutMissedPenalty}
                    attention={playerScoringConfig.shootoutMissedPenalty === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        shootoutMissedPenalty: parseSigned(e.target.value),
                      }))
                    }
                  />
                </Field>
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <IoMdCloseCircle style={{ color: 'rgb(var(--fg))' }} />
                      {t('adminResults.players.scoring.shootoutForcedPenaltyMiss')}
                    </span>
                  }
                >
                  <Input
                    ...
                    inputMode="numeric"
                    min="0"
                    value={playerScoringConfig.shootoutForcedPenaltyMiss}
                    attention={playerScoringConfig.shootoutForcedPenaltyMiss === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        shootoutForcedPenaltyMiss: parsePositive(e.target.value),
                      }))
                    }
                  />
                </Field>
            ...
            <div style={actionGroupStyle}>
              ...
              >
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <GiLeatherBoot style={{ color: '#D4A017', fill: '#D4A017' }} />
                      {t('adminResults.players.scoring.goldenBoot')}
                    </span>
                  }
                >
                  <Input
                    ...
                    attention={playerScoringConfig.award.goldenBoot === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        award: { ...p.award, goldenBoot: parsePositive(e.target.value) },
                      }))
                    }
                  />
                </Field>
                <Field
                  label={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
                      <FaStar style={{ color: '#D4A017', fill: '#D4A017' }} />
                      {t('adminResults.players.scoring.tournamentMvp')}
                    </span>
                  }
                >
                  <Input
                    ...
                    attention={playerScoringConfig.award.tournamentMvp === ''}
                    onChange={(e) =>
                      setPlayerScoringConfig((p) => ({
                        ...p,
                        award: { ...p.award, tournamentMvp: parsePositive(e.target.value) },
                      }))
                    }
                  />
                </Field>
      ...
      {systemMode ? (
        ...
        >
          <PlayerStatsTable
            ...
            onStatChange={(player, stat, delta) =>
              handlePlayerStatChange(player, stat, delta, selectedMatch)
            }
            onAwardToggle={handlePlayerAwardToggle}
            isStatVisible={(player, stat) =>
              isPlayerStatEnabled(playerScoringConfig, player.position, stat)
            }
            toolbar={
              <div
                style={{
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <Field label={t('adminResults.players.matchLabel')} required>
                    <Select<(typeof matchOptions)[number], false>
                      ...
                      onChange={(option) =>
                        setSelectedMatch(
                          option ? { matchId: option.matchId, matchType: option.matchType } : null
                        )
                      }
