  ...
  return (
    <Table minWidth="min(540px, 100%)" maxHeight="65vh" density="compact">
        ...
        <tr>
          ...
          <th style={thStyle}>
            <RankHeaderIconLabel
              ...
              label={t('poolDetail.ranking.groupPhasePoints')}
            />
          </th>
          <th style={thStyle}>
            <RankHeaderIconLabel
              icon={<BsFillDiagram3Fill aria-hidden />}
              label={t('poolDetail.ranking.finalPhasePoints')}
            />
          </th>
          <th style={thStyle}>
            <RankHeaderIconLabel
              icon={<GiSoccerKick aria-hidden />}
              label={t('poolDetail.ranking.playerPoints')}
            />
          </th>
      ...
      <tbody>
        {ranking.map((entry) => {
          const isCurrentUser =
            (entry.userId && entry.userId === currentUserId) || entry.userName === currentUserEmail;

          const rowBackground = isCurrentUser ? 'rgb(var(--accent-from) / 0.08)' : 'transparent';
          ...
          return (
            <tr
              ...
              style={{
                ...
                boxShadow: isCurrentUser ? 'inset 3px 0 0 rgb(var(--accent-from))' : 'none',
            ...
            >
              ...
              <td style={numberStyle}>{entry.groupPhasePoints}</td>

              <td style={numberStyle}>{entry.finalPhasePoints}</td>

              <td style={numberStyle}>{entry.playerPoints}</td>
              ...
              {spyEnabled ? (
                <td style={tdStyle}>
                  <Button
                    ...
                    type="button"
                    onClick={() =>
                      entry.userId && onSpy({ userId: entry.userId, userName: entry.userName })
                    }
...
function RankMovementIndicator({
  ...
  const label = movedUp
    ...
    : movedDown
      ? t('poolDetail.ranking.movement.down', {
          positions: Math.abs(delta),
