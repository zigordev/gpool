    ...
    <main className="container-app" style={{ padding: 24 }}>
      ...
      <section style={{ marginBottom: 28 }}>
        ...
        <p style={{ margin: '0 0 12px', fontSize: '.8rem', color: 'rgb(var(--fg-muted))' }}>
          One implementation. gpool&apos;s loading and icon slots were promoted into the design
          system.
        </p>
        <div
          style={{
        ...
        >
          ...
          <DsButton variant="outline" leadingIcon={<Icon name="plus" size={14} />}>
            Leading
          </DsButton>
          <DsButton variant="outline" trailingIcon={<Icon name="chevron-right" size={14} />}>
            Trailing
          </DsButton>
          <DsButton variant="ghost" size="icon" aria-label="Settings">
            <Icon name="settings" />
          </DsButton>
      ...
      >
        ...
        <DsBadge variant="success" dot>
          Success
        </DsBadge>
        <DsBadge variant="warning" leadingIcon={<Icon name="clock" size={11} />}>
          2d 4h
        </DsBadge>
      ...
      <div style={{ marginBottom: 28 }}>
        <DsButton variant="outline" onClick={() => setModalOpen(true)}>
          Open modal
        </DsButton>
        <Modal
          open={modalOpen}
          ...
          footer={
            <>
              <DsButton variant="ghost" onClick={() => setModalOpen(false)}>
                Cancel
              </DsButton>
              <DsButton variant="danger" onClick={() => setModalOpen(false)}>
                Delete
              </DsButton>
            </>
          }
      ...
      <div style={{ display: 'grid', gap: 12, marginBottom: 28 }}>
        <Section
          ...
          trailing={<DsBadge>12 teams</DsBadge>}
          collapsible
        >
          <p style={{ margin: 0, fontSize: '.85rem', color: 'rgb(var(--fg-muted))' }}>
            Collapsible body.
          </p>
        </Section>
        <Section tone="subtle" density="compact" title="Compact, subtle tone">
          <p style={{ margin: 0, fontSize: '.85rem', color: 'rgb(var(--fg-muted))' }}>
            For nesting inside a card.
          </p>
        </Section>
      </div>
      ...
      <p style={{ margin: '0 0 12px', fontSize: '.8rem', color: 'rgb(var(--fg-muted))' }}>
        Table is presentational only — gpool&apos;s existing .data-table convention, lifted into the
        design system. Sorting and pagination stay with whatever engine a screen uses.
      </p>
        ...
        <Table caption="Standings" maxHeight={220} density="compact">
          <thead>
            <tr>
              <th>#</th>
              <th>Team</th>
              <th className="ds-table-num">Played</th>
              <th className="ds-table-num">Points</th>
            </tr>
          </thead>
          <tbody>
            {[
          ...
          ranking={[
            {
              ...
              finalPhasePoints: 18,
              playerPoints: 9,
              movement: { previousRank: 3, delta: 2, matchdayPoints: 12 },
            },
            {
              rank: 2,
              userId: 'u2',
            ...
            },
            {
              rank: 3,
              userId: 'u3',
              userName: 'Marc Villalonga Puig',
              groupPhasePoints: 38,
              finalPhasePoints: 12,
              playerPoints: 11,
              movement: { previousRank: 2, delta: -1, matchdayPoints: 3 },
            },
          ]}
        />
      </div>
      ...
      <h2 style={{ margin: '0 0 4px', fontSize: '1rem' }}>
        PlayerStatsTable — caption + footer slots
      </h2>
      <p style={{ margin: '0 0 12px', fontSize: '.8rem', color: 'rgb(var(--fg-muted))' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <PlayerStatsTable
          toolbar={
            <DsButton variant="outline" size="sm" leadingIcon={<Icon name="search" size={13} />}>
              Filter players
            </DsButton>
          }
