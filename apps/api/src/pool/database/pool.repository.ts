...
const PLAYER_STAT_COLUMNS = {
  ...
  forcedPenaltyMisses: 'forced_penalty_misses',
...
export class PoolRepository {
  ...
  async updateTeamEliminationState(survivingTeamIds: string[]) {
    const result = await this.postgres.query(
      `
        UPDATE teams
        SET eliminated = CASE
          WHEN array_length($1::text[], 1) IS NULL THEN FALSE
          ELSE NOT (team_id = ANY($1::text[]))
        END
        RETURNING
          team_id AS "teamId",
          name,
    ...
    const result = await this.postgres.query(
      `
        SELECT
          ...
          COALESCE(teams.eliminated, FALSE) AS "teamEliminated",
          tournament_players.shirt_number::int AS "shirtNumber",
          ...
          COALESCE(s.forced_penalty_misses, 0)::int AS "forcedPenaltyMisses",
  ...
  async getTournamentPlayerMatchStats(playerId: string) {
    const result = await this.postgres.query(
      `
        SELECT *
        FROM (
          SELECT
            ms.match_type AS "matchType",
            ...
            g.home_result AS "homeResult",
            g.away_result AS "awayResult",
            g.scheduled_at::text AS "scheduledAt",
            ...
            ms.forced_penalty_misses::int AS "forcedPenaltyMisses",
          ...
          INNER JOIN group_phase_matches g
            ON ms.match_type = 'group'
            AND g.pool_id = 'all-pools'
            AND g.match_id = ms.match_id
          ...
          SELECT
            ...
            f.home_result AS "homeResult",
            f.away_result AS "awayResult",
            f.scheduled_at::text AS "scheduledAt",
            ms.goals::int AS goals,
            ms.penalty_goals::int AS "penaltyGoals",
            ms.missed_penalties::int AS "missedPenalties",
            ms.mvps::int AS mvps,
          ...
          INNER JOIN final_phase_matches f
            ON ms.match_type = 'final'
            AND f.pool_id = 'all-pools'
            AND f.bracket_match_id = ms.match_id
          WHERE ms.player_id = $1
        ) matches
        WHERE
          goals > 0
          OR "penaltyGoals" > 0
          OR "missedPenalties" > 0
          OR mvps > 0
          OR "penaltiesSaved" > 0
          OR "forcedPenaltyMisses" > 0
      ...
      const totalsResult = await client.query(
        `
          ...
          RETURNING
            ...
            forced_penalty_misses::int AS "forcedPenaltyMisses",
    ...
    const result = await this.postgres.query(
      `
        SELECT
          ...
          COALESCE(t.eliminated, FALSE) AS "teamEliminated",
          p.shirt_number::int AS "shirtNumber",
          COALESCE(ms.goals, 0)::int AS goals,
          COALESCE(ms.penalty_goals, 0)::int AS "penaltyGoals",
          COALESCE(ms.missed_penalties, 0)::int AS "missedPenalties",
          COALESCE(ms.mvps, 0)::int AS mvps,
          COALESCE(ms.penalties_saved, 0)::int AS "penaltiesSaved",
          COALESCE(ms.forced_penalty_misses, 0)::int AS "forcedPenaltyMisses",
        ...
        LEFT JOIN tournament_player_match_stats ms
          ON ms.player_id = s.player_id
          AND ms.match_type = $2
          AND ms.match_id = $3
        WHERE s.pool_id = $1
          AND p.team_id = ANY($4::text[])
        ORDER BY s.user_id ASC, s.position ASC, s.slot ASC
      `,
      [poolId, matchType, matchId, teamIds]
    );
    ...
    const result = await this.postgres.query(
      `
        SELECT
          ...
          COALESCE(ps.forced_penalty_misses, 0)::int AS "forcedPenaltyMisses",
    ...
    const result = await this.postgres.query(
      `
        ...
        SELECT
          ...
          COALESCE(SUM(ms.forced_penalty_misses), 0)::int AS "forcedPenaltyMisses",
  ...
  async upsertPlayerAwardSelection(
    poolId: string,
    userId: string,
    award: string,
    playerId: string
  ...
  async deletePrediction(poolId: string, matchId: string, userId: string) {
    ...
    return {
      predictionId: result.rows[0]?.predictionId || null,
      poolId,
      matchId,
      userId,
      cleared: true,
  ...
  async deleteBracketMatch(bracketMatchId: string) {
    await this.postgres.query(`DELETE FROM final_phase_matches WHERE bracket_match_id = $1`, [
      bracketMatchId,
    ]);
    return { success: true };
  }
