...
export default function GroupsPage() {
  ...
  const renderMatchCard = (match: Match) => {
    ...
    const isCorrectWinner =
      isPastDeadline && hasResults && prediction.isCorrect === true && !isExactMatch;
    const hasUserPrediction = prediction.homeScore !== '' && prediction.awayScore !== '';
    const isIncorrect =
      isPastDeadline && hasResults && hasUserPrediction && prediction.isCorrect === false;
    const isIncomplete =
      !isPastDeadline && (prediction.homeScore === '' || prediction.awayScore === '');
    ...
    } else if (hasResults && !hasUserPrediction && isPastDeadline) {
      state = 'pending';
...
function PredictionStandingsTable({
  ...
  const columns = [
    {
      key: 'position',
      label: t('poolDetail.groupPhase.standings.position'),
    ...
    ...(showGroup
      ? [
          {
            key: 'group',
            label: t('poolDetail.groupPhase.standings.group'),
            align: 'center' as const,
          },
        ]
      : []),
    ...
    {
      key: 'goalsFor',
      label: t('poolDetail.groupPhase.standings.goalsFor'),
      align: 'right' as const,
    },
    {
      key: 'goalsAgainst',
      label: t('poolDetail.groupPhase.standings.goalsAgainst'),
      align: 'right' as const,
    },
    {
      key: 'goalDifference',
      label: t('poolDetail.groupPhase.standings.goalDifference'),
      align: 'right' as const,
    },
    ...(showStatus
      ? [
          {
            key: 'status',
            label: t('poolDetail.groupPhase.standings.status'),
            align: 'center' as const,
          },
        ]
      : []),
        ...
        {rows.map((row, index) => {
          ...
          return (
            <tr key={`${row.group}-${row.teamId}`}>
              <td
                style={{
                  ...standingsStickyCellStyle('position'),
                  textAlign: 'center',
                  fontWeight: 800,
              ...
              <td
                style={{ ...standingsStickyCellStyle('team'), textAlign: 'left', fontWeight: 700 }}
              >
                <span
                  style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', minWidth: 0 }}
                >
                  <ReactCountryFlag
                    countryCode={countryIsoCode(row.name)}
                    svg
                  ...
                  >
                    {countryDisplayName(row.name, t)}
                  </span>
                </span>
              </td>
              {showGroup ? (
                <td
                  style={{ ...standingsColumnStyle('group'), textAlign: 'center', fontWeight: 800 }}
                >
                  {row.group}
                </td>
              ) : null}
              ...
              {showStatus ? (
                <td style={{ textAlign: 'center' }}>
                  ...
                  >
                    {qualified
                      ? t('poolDetail.groupPhase.standings.qualified')
                      : t('poolDetail.groupPhase.standings.eliminated')}
                  </span>
                </td>
              ) : null}
            </tr>
