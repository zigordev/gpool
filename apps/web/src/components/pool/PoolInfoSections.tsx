...
export function GeneralPoolInfoSection({
  ...
  return (
    <PoolDetailModalButton
      title={t('poolDetail.rules.poolConfig.title')}
      icon={<FaInfo size={13} />}
    >
              ...
              >
                {prizeDistribution.map((payout) => (
                  ...
                  >
                    ...
                    <span>
                      {formatCurrency ? formatCurrency(payout.amount) : `${payout.amount}€`}
                    </span>
                  </span>
                ))}
              </div>
          ...
          return (
            ...
            >
              ...
              <span style={{ marginLeft: '0.75rem' }}>
                ...
                <PointValue
                  points={round.correctTeamWrongPositionPoints}
                  color="rgb(var(--info))"
                />
              </span>
            </p>
...
export function PlayerScoringInfoSection({
  ...
  const visibleActionGroups = actionGroups.filter(
    (group) =>
        ...
        {visibleActionGroups.map((group, groupIndex) => (
          ...
          >
            ...
            {visiblePositionRows
              ...
              .map((row) => {
                const visiblePositions = PLAYER_POSITIONS.filter((position) =>
                  isVisibleScoringValue(row.values[position.key])
                );
                return (
...
export function resolvePlayerInfoScoring(value: any) {
  return {
    ...
    forcedPenaltyMiss: Number.isFinite(Number(value?.forcedPenaltyMiss))
      ? Number(value.forcedPenaltyMiss)
      : DEFAULT_PLAYER_RULE_SCORING.forcedPenaltyMiss,
    shootoutPenaltySaved: Number.isFinite(Number(value?.shootoutPenaltySaved))
      ? Math.max(0, Number(value.shootoutPenaltySaved))
      : DEFAULT_PLAYER_RULE_SCORING.shootoutPenaltySaved,
    shootoutGoal: Number.isFinite(Number(value?.shootoutGoal))
      ? Math.max(0, Number(value.shootoutGoal))
      : DEFAULT_PLAYER_RULE_SCORING.shootoutGoal,
    shootoutMissedPenalty: Number.isFinite(Number(value?.shootoutMissedPenalty))
      ? Number(value.shootoutMissedPenalty)
      : DEFAULT_PLAYER_RULE_SCORING.shootoutMissedPenalty,
    shootoutForcedPenaltyMiss: Number.isFinite(Number(value?.shootoutForcedPenaltyMiss))
      ? Number(value.shootoutForcedPenaltyMiss)
      : DEFAULT_PLAYER_RULE_SCORING.shootoutForcedPenaltyMiss,
    ...
    doubleYellowCard: Number.isFinite(Number(value?.doubleYellowCard))
      ? Number(value.doubleYellowCard)
      : DEFAULT_PLAYER_RULE_SCORING.doubleYellowCard,
    redCard: Number.isFinite(Number(value?.redCard))
      ? Number(value.redCard)
      : DEFAULT_PLAYER_RULE_SCORING.redCard,
    award: {
...
function resolvePositionScoring<T extends Record<PlayerPosition, number>>(
  ...
  return {
    goalkeeper: Number.isFinite(Number(value?.goalkeeper))
      ? Number(value.goalkeeper)
      : (legacyFallback ?? fallback.goalkeeper),
    defender: Number.isFinite(Number(value?.defender))
      ? Number(value.defender)
      : (legacyFallback ?? fallback.defender),
    midfielder: Number.isFinite(Number(value?.midfielder))
      ? Number(value.midfielder)
      : (legacyFallback ?? fallback.midfielder),
    forward: Number.isFinite(Number(value?.forward))
      ? Number(value.forward)
      : (legacyFallback ?? fallback.forward),
  } as T;
}
